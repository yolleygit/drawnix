...
- **ai-generation:** 完整 OpenRouter API 支持
  - 支持 OpenRouter 平台访问 Gemini 2.5 系列模型
  - 智能 API 类型检测和自动切换
  - 完整的向后兼容性保证
- **ai-generation:** 智能模型格式迁移
  - 自动检测和转换模型名称格式
  - OpenRouter 格式: `google/gemini-2.5-flash-image-preview`
  - 原生格式: `gemini-2.5-flash-image-preview`
  - 无缝迁移用户现有配置
- **ai-generation:** 多路径探测和缓存机制
  - 智能探测不同 API 端点格式
  - 缓存有效路径提高性能
  - 自动错误处理和重试
- **ai-generation:** 增强错误处理
  - 用户友好的错误提示
  - 详细的调试日志输出
  - 智能错误诊断和解决建议
...
- **ai-generation:** 修复 OpenRouter API 端点错误
  - 更正 API 端点为 `/api/v1/chat/completions`
  - 修复 CORS 问题和请求格式
- **ai-generation:** 修复模型选择与调用不匹配问题
  - 统一模型名称选择和实际调用逻辑
  - 解决设置保存格式与 API 调用格式不一致
- **ai-generation:** 修复 OpenRouter 响应解析
  - 支持 `message.images[0].image_url.url` 字段格式
  - 增强多种响应格式的兼容性

### 📚 Documentation

- 新增完整的 OpenRouter 集成文档
- 更新 README.md 添加 AI 绘图功能详细说明
- 提供两种 API 配置方式的详细步骤
- 添加技术实现细节和故障排除指南
